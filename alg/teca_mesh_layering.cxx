    ...
    size_t nz, size_t ny, size_t nx)
{
    for (size_t k = 0; k < nz; ++k)
    {
        for (size_t j = 0; j < ny; ++j)
        {
            ...
            {
                output[k*ny*nx + jj + i] = input[jj + i];
            }
        }
    }
}
    ...
    const std::string &prefix, options_description &global_opts)
{
    ...
    opts.add_options()
        TECA_POPTS_GET(size_t, prefix, n_layers,
...
void teca_mesh_layering::set_properties(const std::string &prefix,
    variables_map &opts)
{
    TECA_POPTS_SET(opts, size_t, prefix, n_layers)
}
#endif
    ...
    const teca_metadata &request)
{
    ...
    if (n_layers > 1)
    {
        ...
        if (n_z > 0)
        {
            TECA_ERROR("Cannot layer 3D arrays")
            return nullptr;
        }
        ...
        req_whole_extent[5] = n_layers;
        req_extent[1] = n_x;
        req_extent[3] = n_y;
        req_extent[5] = n_layers;
        ...
        TEMPLATE_DISPATCH_FP(
            ...
            teca_algorithm_util::expand_dimension(p_z_crds_new, p_z_crds, n_z,
                            0, n_layers);
        ...
        for (size_t i = 0; i < n_arrays; ++i)
        {
            ...
            TEMPLATE_DISPATCH_FP(
                ...
                out_mesh->get_point_arrays()->set(i, layered_array);
            )
        }
    }
